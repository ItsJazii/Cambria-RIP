---
title: "1,920 Players Sorted Into Three Buckets"
date: 2026-08-14
dateLabel: "Aug 14, 2026"
category: "Exploit Fix"
summary: >-
  The second-pass resolution split affected accounts by how they played: 1,315 normal fishers got 100% of their bugged keys airdropped, 354 unclear cases got 50% Grub Credit, and 251 accounts judged to be farming the T1 edge got 25% and kept their adjustment.
sources:
  - label: "Aug 13: Emerald Bug"
    url: "https://blog.cambria.gg/aug-13-emerald-bug/"
  - label: "Cambria Discord"
    url: "https://discord.gg/playcambria"
---

The first pass gave everyone 25% Grub Credit. The second pass sorted 1,920 accounts by behaviour during the bug window:

- **1,315 players fishing normally** - 100% of bugged keys airdropped back, all of Mirelle's Adjustments cleared, refunds for anyone who had already handed keys back.
- **354 players the team could not confidently categorise** - 50% Grub Credit, double the original, plus adjustments cleared.
- **251 players judged to be specifically farming the T1 emerald edge** - 25% Grub Credit and the adjustment stands, netted against their airdrop allocation.

184 players had withdrawn significantly more than their corrected balance and could not be fully reversed; those differences became "Mirelle's Adjustments" on their receipt, deducted from their airdrop at launch. Total Grub Credit issued: $125,000, funded from fees and uplift. A share of keys was temporarily locked as Key Vouchers, redeeming 1:1 on a weekly cycle from August 20th.

All Genesis Points earned during the bug window were kept, with a bonus planned on top for regular players who did not mass farm the edge.

Being told an algorithm has classified you as an exploiter, with your token allocation as the collateral, is a genuinely new category of MMO experience.
